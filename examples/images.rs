...
fn images_ui(ui: &mut egui::Ui, handle: &egui::TextureHandle) {
...
impl eframe::App for ImagesApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        let handle = self
            .handle
            .get_or_insert_with(|| make_checkerboard(ui.ctx()))
            .clone();
        images_ui(ui, &handle);
    }
}
...
fn main() -> eframe::Result<()> {
    eframe::run_native(
        ...
        Box::new(|_cc| Ok(Box::new(ImagesApp { handle: None }))),
    )
}
