...
- **`DesignValue` marker trait** (`'static + Clone + Send + Sync`, blanket-implemented for every type that satisfies it) replaces the repeated 3-4-part bound list on every `DesignSlots` method (`set_design_data`, `design_data`, `design`) and the `Context` impl. Non-breaking - existing calls are unaffected since any type that satisfied the old bounds still satisfies `DesignValue`.
